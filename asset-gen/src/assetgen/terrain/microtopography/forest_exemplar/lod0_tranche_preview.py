"""Pack one complete accepted forest LOD0 tranche as a format-2 preview."""
...
def _artifact_files(artifact_root: Path, master_bbox: tuple[int, int, int, int]) -> dict[str, str]:
    ...
    if (
        ...
        raise ValueError("forest LOD0 tranche is not the accepted float artifact")
    for relative, identity in manifest["files"].items():
        path = artifact_root / relative
        if path.stat().st_size != identity["bytes"] or _sha256(path) != identity["sha256"]:
            raise ValueError(f"forest LOD0 tranche artifact changed: {relative}")
    ...
    if master.shape != MASTER_SHAPE or master.dtype != np.float32:
        raise ValueError("forest LOD0 tranche master must be float32 32768 square")
...
def _recipe_identity(
    ...
    source_paths = (
        Path(__file__),
        Path(__file__).with_name("lod0_tranche_preview_verify.py"),
    ...
    for site in geom.sites:
        ...
        for tile_row in range(4):
            for tile_col in range(4):
                ...
                packed[rows, cols] = np.packbits(mask.allowed, axis=1, bitorder="little")
...
def _stage_fine(
    ...
    def core(chunk: HeightChunkId) -> np.ndarray:
        ...
        if not path.exists():
            temporary = path.with_suffix(".tmp.npy")
            np.save(temporary, _baseline_core(pinned, chunk), allow_pickle=False)
    ...
    total = len(coverage.published_fine) + len(coverage.transient_support)
    for index, chunk in enumerate((*coverage.published_fine, *coverage.transient_support), 1):
        ...
        identity = _write_height(destination, base, chunk, values, FINE_QSCALE, qoffset=30.0)
        identity["role"] = "contiguous-accepted-master" if chunk in published else "base-apron-support"
        paths[chunk] = destination
        identities.append(identity)
        if index % 64 == 0 or index == total:
            print(f"[forest-tranche-pack] fine {index}/{total}", flush=True)
...
def _stage_parents(
    ...
    for index, site in enumerate(geom.sites, 1):
        ...
        identity = _write_height(
            ...
            base,
            site.parent,
            box_mean4_striped(mosaic),
            PARENT_QSCALE,
            qoffset=TRANCHE_QOFFSET_M,
        )
        identity["siteId"] = site.site_id
        identities.append(identity)
        decoded[site.parent] = _decoded(destination, base)
        print(f"[forest-tranche-pack] parent {index}/16", flush=True)
...
def _unpack_allowed(path: Path) -> np.ndarray:
    ...
    if packed.shape != (8192, 1024) or packed.dtype != np.uint8:
        raise ValueError(f"invalid staged forest tranche mask: {path}")
...
def materialize_lod0_tranche_preview(
    *,
    ...
    evidence = {
        "format": 1,
        "cook": "accepted-forest-lod0-tranche-absolute-master-v1",
    ...
    _immutable(build_root / "evidence/forest-lod0-tranche-preview.json", _json_bytes(evidence))
    create_build_plan(
        base,
        build_digest,
        COOK_REVISION,
...
def main() -> None:
    ...
    print(materialize_lod0_tranche_preview(**vars(parser.parse_args())))

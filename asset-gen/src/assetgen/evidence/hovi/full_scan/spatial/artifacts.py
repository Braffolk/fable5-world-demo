...
def _file_sha256(path: Path, expected_bytes: int) -> str:
    descriptor = open_regular_nofollow(path, f"spatial artifact {path.name}")
    try:
        ...
        if (
            ...
            raise PermissionError("Hovi spatial artifact is not the private staging inode")
        ...
        if total != expected_bytes or identity_before != identity_after:
            raise ValueError("Hovi spatial artifact changed while being hashed")
...
def _tree_inventory(root: Path) -> tuple[set[str], set[str]]:
    ...
    directories: set[str] = {"."}
    files: set[str] = set()
    for current, names, filenames, _ in os.fwalk(root, follow_symlinks=False):
        ...
        for name in names:
            ...
            if (
                ...
                raise PermissionError("Hovi spatial staging contains a non-private directory")
            directories.add(path.relative_to(root).as_posix())
        for name in filenames:
            path = current_path / name
            result = os.lstat(path)
            if stat.S_ISLNK(result.st_mode) or not stat.S_ISREG(result.st_mode):
                raise PermissionError("Hovi spatial staging contains a non-regular file")
...
def verify_spatial_staging(
    ...
    directories, files = _tree_inventory(staging_root)
    expected_files = {artifact.relative_path for artifact in artifacts}
    existing_manifest = "manifest.json" in files
    allowed_files = expected_files | ({"manifest.json"} if existing_manifest else set())
    if directories != expected_directories or files != allowed_files:
        raise ValueError("Hovi spatial staging tree contains unreported entries")

    for artifact in artifacts:
        if (
            _file_sha256(staging_root / artifact.relative_path, artifact.bytes)
            != artifact.sha256
        ):
            raise ValueError(f"Hovi spatial artifact hash differs: {artifact.relative_path}")
    ...
    manifest = canonical_json_bytes(
        {
            ...
            "authority": {
                "configPath": authority.config_path.as_posix(),
    ...
    if existing_manifest:
        manifest_descriptor = open_regular_nofollow(
            staging_root / "manifest.json",
            "existing Hovi spatial acceptance manifest",
        )
        try:
            result = os.fstat(manifest_descriptor)
            payload = bytearray()
            while block := os.read(manifest_descriptor, 1 << 20):
                payload.extend(block)
            if (
                result.st_uid != os.getuid()
                or stat.S_IMODE(result.st_mode) != 0o400
                or result.st_nlink != 1
                or bytes(payload) != manifest
            ):
                raise ValueError("existing Hovi spatial manifest differs from verification")
        finally:
...
def publish_verified_spatial_staging(
    ...
    current = os.lstat(verified.root)
    current_identity = (
        current.st_dev,
        current.st_ino,
        current.st_size,
    ...
    if current_identity != verified.root_identity:
    ...
    root_fd = materializations = sha_directory = prefix_directory = -1
    try:
        try:
            manifest_fd = os.open(
                "manifest.json",
            ...
            manifest_fd = open_regular_nofollow(
                verified.root / "manifest.json",
                "existing Hovi spatial acceptance manifest",
            )
            try:
                payload = bytearray()
                while block := os.read(manifest_fd, 1 << 20):
                    payload.extend(block)
                result = os.fstat(manifest_fd)
                if (
                    bytes(payload) != verified.manifest
                    or stat.S_IMODE(result.st_mode) != 0o400
                    or result.st_nlink != 1
                ):
        ...
        for artifact in verified.artifacts:
            descriptor = open_regular_nofollow(
                verified.root / artifact.relative_path,
                f"verified spatial artifact {artifact.relative_path}",
            )
            try:
                ...
                if (
                    ...
                    or result.st_nlink != 1
                    or result.st_size != artifact.bytes
                ):
                    raise PermissionError("Hovi spatial artifact changed before finalization")

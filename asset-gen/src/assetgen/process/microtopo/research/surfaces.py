...
def failed_hovi_surface(bundle: FrozenBundle, semantic: SemanticResult) -> ResearchSurfaceEvidence:
    ...
    surface = ResearchSurfaceEvidence(
        ...
        view_count=np.zeros(shape, dtype=np.uint8),
        interpolation_distance_m=nan.copy(),
        semantic_probabilities=np.zeros((*shape, 6), dtype=np.float32),
        ...
        group_a_support=np.zeros(shape, dtype=np.uint8),
        group_b_support=np.zeros(shape, dtype=np.uint8),
...
def adapt_evo(bundle: FrozenBundle, build_id: str, plot: str) -> ResearchSurfaceEvidence:
    ...
    with np.load(path) as source:
        ...
        surface = ResearchSurfaceEvidence(
            ...
            view_count=source["view_group_count"].astype(np.uint8),
...
def save_surface(path: Path, surface: ResearchSurfaceEvidence) -> dict:
    ...
    with path.open("wb") as target:
        np.savez_compressed(
            ...
            view_count_u8=surface.view_count,
            ...
            group_a_support_u8=surface.group_a_support,
            group_b_support_u8=surface.group_b_support,

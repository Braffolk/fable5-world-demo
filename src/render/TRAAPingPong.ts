// @ts-nocheck — VERBATIM FORK of three r184 examples/jsm/tsl/display/TRAANode.js
// (RP-3a, deep-review 08-L2a / 16-L7): PING-PONG history. The stock node renders the
// resolve into _resolveRenderTarget then does a FULL-RES copyTextureToTexture back
// into _historyRenderTarget every frame (~26.7MB of pure bandwidth + an encoder op).
// Here the two RTs alternate write/read roles — the resolve IS the next history; the
// color copy is DELETED. The history sampler + output passTexture swap .value each
// frame (three's own AfterimageNode ping-pong pattern). Everything else — jitter
// sequence, setViewOffset flow, _jitterIndex (NaniteFrame mirrors it), the velocity
// seam, prev-DEPTH copy (still needed; depth home stays on rtA) — is stock-verbatim.
...
class TRAANode extends TempNode {
	...
	constructor( beautyNode, depthNode, velocityNode, camera ) {
		...
		/**
		 * PING-PONG (fork): the two color RTs alternate roles each frame. _readRT is
		 * sampled as history by the resolve material (via _historyNode, whose .value
		 * swaps); _writeRT receives the resolve and BECOMES next frame's history —
		 * no resolve→history color copy. The prev-DEPTH texture home stays fixed on
		 * _historyRenderTarget (rtA) regardless of role.
		 *
		 * @private
		 */
		this._readRT = this._historyRenderTarget;
		this._writeRT = this._resolveRenderTarget;

		/**
		 * PING-PONG (fork): the swappable history sampler (stock creates a local
		 * `texture(...)` inside setup(); we need a handle to swap .value per frame).
		 *
		 * @private
		 * @type {TextureNode}
		 */
	...
	updateBefore( frame ) {
		...
		// resolve — PING-PONG (fork): render into the WRITE role (sampling the READ role
		// as history via _historyNode), then swap roles: the resolve IS the new history.
		// The stock full-res resolve→history copyTextureToTexture is DELETED.
		...
		// downstream consumers sample THIS frame's resolve
		this._textureNode.value = this._writeRT.texture;

		// swap roles for next frame; history sampler follows the new READ role
		const swap = this._readRT;
	...
	setup( builder ) {
		...
		const historyNode = this._historyNode; // PING-PONG (fork): swappable — see updateBefore
...
export { TRAANode as TRAAPingPongNode };
 ...
 * @returns {TRAANode}
 */
export const traaPingPong = ( beautyNode, depthNode, velocityNode, camera ) => new TRAANode( convertToTexture( beautyNode ), depthNode, velocityNode, camera );

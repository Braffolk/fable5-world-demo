...
export function buildNaniteRaster(
  ...
  grass?: {
    ...
    enabled(): boolean;
    /** Non-null only for shell-free translated-surface terrain reconstruction. */
    envelopeQuery: {
      height: number;
  ...
  const hw = buildHw({
    ...
    indices: gpu.indices,
    hwproj,
    grassEnvelope: grass?.envelopeQuery ?? null,
  });
  const {
    kHwArgs,
    kGrassEnvelopeArgs,
  ...
  const world1 = (
    ...
    dispatchBatchMixed(renderer, [
      ...
      ...(kGrassEnvelopeArgs ? [kGrassEnvelopeArgs as unknown] : []),
    ...
    // Query unchanged terrain from the shifted camera before grass consumes the
    // native depth + exact winning chart. This target is offscreen and cannot
    // create a scene-visible duplicate surface.
    if (grass?.enabled()) hwRenderGrassEnvelope(renderer, camera);

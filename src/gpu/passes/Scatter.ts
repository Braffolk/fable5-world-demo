...
/**
 * Sample a canopy coverage texture at a world xz (filtered, world-UV mapping).
 * Since S4 the texture is the CanopyWindow's — this tap is exact while the
 * window is pinned over the whole world (the generated source); consumers that
 * must survive a MOVING window (S5 Estonia roam) use `CanopyWindow.covAt`.
 */

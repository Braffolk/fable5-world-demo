...
### Post-park check: literal PCF-like order statistics

A final source-interpretation check evaluated fixed four-corner vertical-drop
order statistics on the original dense 53,248-ray domain. A front quartile and
weighted median retain the baseline's identical 65.68% hit agreement and
96.25% recall because they cannot repair coverage disagreement. Their
world-position p95/p99 errors are respectively `4.769/7.588 m` and
`4.643/7.542 m`, both worse than projected relift's `4.137/7.060 m`.

The concrete four-read PCF interpretations are rejected. They add four-input
sorting/select ALU and record live ranges without repairing line-space
topology. The preserved report is
`data/work/groundcover-ray-surrogate/2ed57f59d86e8376/0ca11c87930de1f9/report.json`
(SHA-256
`0ca11c87930de1f9bda6d4b2e9e2ce9b3ad64ad327a3a2d1a4cdd1b5cd98bc49`).
This is deliberately not presented as Sannikov's unpublished algorithm.

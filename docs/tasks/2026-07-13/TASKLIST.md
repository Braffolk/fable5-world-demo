...
- [ ] Correct and independently audit terminal bit-padding validation, bind a new immutable native build/selection, then restart the 16-scan materialization. The first attempt under selection `5e8373c88196d539f614a65215e7a594aabc6297c636fe8946b2ac6b8288c91f` failed closed after scan 0 because the verifier treated legal byte-alignment padding as an extra narrow field code; its 5.3 GiB staging tree is rejected evidence, not a materialization. The restart must still cap decoding at `4,248,797,321` declared records, accept only bounded zero padding, and reject nonzero trailing data.
...
- [x] Bound every integer unpack by the declared remaining record budget and reject terminal residual bits that can still encode a full code, so final-byte padding cannot become an extra narrow integer record.
- [ ] Finish, independently verify, and atomically publish the active one-pass scan-preserving materialization. Any timeout, reserve breach, count/routing mismatch, duplicate source ordinal, ABI/domain failure, unreported file, identity drift, or manifest mismatch fails closed.
- [ ] Convert that materialization into a view-aware candidate surface with scan-balanced multiscale sheet evidence, unique-view/angular support, occlusion/interpolation masks, inter-scan error, full-versus-thinned transfer analysis, explicit semantic unknowns, and labeled PNG QA. Keep the result unqualified and synthesis-forbidden unless the later gates pass.

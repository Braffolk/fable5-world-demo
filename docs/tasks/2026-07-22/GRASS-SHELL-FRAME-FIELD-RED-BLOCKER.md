...
The shell-frame field's central empirical premise does not hold for the
expected-easy authored Agrostis community. Exterior first hits do **not**
...
- `316` finite rings;
- `175,940` directions total, versus the required `<=97` dense-community
  baseline;
...
The paper design must replace at least one load-bearing premise; implementation
tuning cannot fix this result. A revised model needs one of:
...
- Exact blocker: the expected-easy community emits `175,940`, not `<=97`,
  directions because measured crisp residual is about `0.20 m`, not
  millimetric or grazing-decreasing.

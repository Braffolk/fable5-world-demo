...
**Status snapshot (2026-07-12):** HEAD `d584227`; data manifest `m/843ed7c85cc1370b` (cook_rev 3,
...
| 114 | Water/shoreline BORDERS render as blocky "big squares" → smooth (ASSET-GEN) | ⏳ SCOPING (w/ #104) | cook_water rasterizes ETAK water polys to a HARD grid → boundary quantized to cells. Fix asset-gen side: supersample + anti-aliased COVERAGE FRACTION (or shoreline distance) instead of binary wet mask → smooth edge under bilinear. Folded into the #104 water scoping (shared cooker/layer). |
| 104 | Water bathymetry: bed coplanar w/ terrain (z-fight); needs REAL depth | ⏳ SCOPING | **USER DECISION = option C** (physics + real banks: ETAK width/type hydraulic-geometry + LiDAR bank wedge for ungauged rivers/lakes; real gridded bathymetry — Maa-amet HIS / EMODnet / BSBD sea, TLU/KAUR lakes — where it exists; NEVER fake). Research done. Pre-cook groundwork: confirm Maa-amet download packaging + datum offset, KAUR lake-raster download path, pin EE width→depth coefficients. Split: asset-gen depth-layer cook + runtime submerged-bed draw. |

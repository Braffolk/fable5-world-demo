...
**Status snapshot (2026-07-12):** HEAD `4523438`; data manifest `m/fffd3771349f27c7` (cook_rev 4:
...
| 116 | Wire cooked SOIL taxonomy (soilType/stoniness/boniteet) — follow-up to #111 | 🔧 IN FLIGHT | Design spec ready (`scratchpad/SOIL-WIRING-116-SPEC.md`): cooked soil = 5 u8 planes (soilType/texCore/texSkeleton/stoniness/boniteet), pilot-scale LOD0 only 4.87 MB. Plan: pack 4 into one rgba8 plane (texCore→mineral tint, stoniness→geometric speckle, boniteet→litter richness, texSkeleton→hue), DROP soilType (covered by #111 classId). Plumbing mirrors watercover (StreamProtocol/PlaneFill/StreamBrainCore/TerrainField); **gotcha:** `StreamBrainClient.buildInit` hardcodes a layer list omitting `'soil'` — must add or plane fills zero. Generated bit-identical (`GeneratedWorldSource` has no soil ⇒ `hasSoil=false`). VRAM ≤4 MiB. |

...
| **N9-C2 — leaf aggregate cut wired + the per-instance FLOOD measured** (2026-06-15, probe-leafzoom, shot 7 forest interior, `?naniteleaf=1` density 4000, 1280×720, freeze, terrain-DAG/shadow/occl OFF, NOT cooled) | — | — | — | — | — | — | The leaf aggregate (`BuildAggregateDag`) is attached + the envelope extended → TREE_GEO_FAR (496 m); the GPU cut is VALIDATED (probe-leafzoom: τ-monotonic **2.8k→3.76M** leaf clusters over τ 32→0.25, smooth zoom, no errors). BUILD COST: 20 crowns build SYNC in **15.5 s @ density 4000** (5.4 s @ 800) ⇒ over the D6 world-gen budget ⇒ the build must move to the Worker/time-slice path (D-N30; pending). **THE FLOOD @ τ=1: frame 133 ms, 1.89M visible clusters (1.68M leaf), 1.13M crown instances, 14.7M tris, 75k hwTris.** 1.68M/1.13M ≈ **1.5 clusters/instance ⇒ the O(instances) per-instance floor** (D-N35/D-N41), NOT DAG depth ⇒ **N8-HIC FORCED** (pulled into C2). Leaf path is OPT-IN; default bit-identical. The Worker build + the two-sided raster (drop the geometry-dup, ≈½ the leaf geometry) are tracked leaf-perf follow-ons bundled with the N8-HIC push. |

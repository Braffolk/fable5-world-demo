...
import {
  ...
  integrateRealFourierSeriesMoments,
  integrateQuadraticWindowedRealFourierSeriesMoments,
  liftClassEFirstPassage,
  liftClassECurveFirstPassage,
...
test('filled-solid reference pole uses occupancy and no epsilon', () => {
  const affine: ClassEAffine3 = {
...
test('cap-free curve query keeps arc crossings and handles exact projected-pole coincidence', () => {
  const identity: ClassEAffine3 = {
    linear: [1, 0, 0, 0, 1, 0, 0, 0, 1],
    translation: [0, 0, 0],
  };
  const ray = toCanonicalRay(identity, [-1, 0, 0], normalized([1, 0, 0.2]));
  const interval = clipCanonicalAxialInterval(ray, -0.4, 0.9, 20)!;
  // A compact arc is a two-sided sheet, not a filled solid: there is only a
  // categorical first curve crossing, with no entry/exit state.
  const lateral = liftClassECurveFirstPassage(ray, interval, 0.2);
  close(lateral!, interval.tMin + 0.2 / ray.projectedSpeed);

  const pole = toCanonicalRay(identity, [0.1, -1, 0.2], [0, 0, 1]);
  const poleInterval = clipCanonicalAxialInterval(pole, -10, 10, 3)!;
  assert.equal(liftClassECurveFirstPassage(pole, poleInterval, null, false), null);
  close(liftClassECurveFirstPassage(pole, poleInterval, null, true)!, poleInterval.tMin);
});
...
test('stored tangent line exactly reintersects every live ray in one straight segment cell', () => {
  ...
  assert.equal(tangentReintersection(hit, normal, [0, 0], [1e-3, 1], 1e-2), null);
});
...
test('pole-stable Fourier residual optical depth and centroid match quadrature', () => {
  const constant = 0.9;
  const coefficients = [
    { coefficientReal: 0.11, coefficientImaginary: -0.07, phaseAtZero: 0.31 },
    { coefficientReal: -0.05, coefficientImaginary: 0.03, phaseAtZero: -0.73 },
  ] as const;
  for (const speed of [0, 1e-10, 1e-5, 0.7, -2.3]) {
    const modes = coefficients.map((coefficient, index) => ({
      ...coefficient,
      angularSpeed: speed * (index + 1.4),
    }));
    const t0 = -0.4;
    const t1 = 1.8;
    const exact = integrateRealFourierSeriesMoments(constant, modes, t0, t1);
    const steps = 200_000;
    const dt = (t1 - t0) / steps;
    let opticalDepth = 0;
    let firstMoment = 0;
    for (let index = 0; index < steps; index++) {
      const t = t0 + (index + 0.5) * dt;
      let density = constant;
      for (const mode of modes) {
        const phase = mode.phaseAtZero + mode.angularSpeed * t;
        density += 2 * (
          mode.coefficientReal * Math.cos(phase)
          - mode.coefficientImaginary * Math.sin(phase)
        );
      }
      opticalDepth += density * dt;
      firstMoment += t * density * dt;
    }
    close(exact.opticalDepth, opticalDepth, 2e-9);
    close(exact.firstMoment, firstMoment, 3e-9);
    close(exact.centroid!, firstMoment / opticalDepth, 3e-9);
  }
});

test('quadratic-windowed Fourier residual matches independent dense quadrature at poles and ordinary slopes', () => {
  const constant = 1.1;
  const weight = [0.08, 0.73, -0.21] as const;
  const coefficients = [
    { coefficientReal: 0.09, coefficientImaginary: -0.04, phaseAtZero: 0.17 },
    { coefficientReal: -0.03, coefficientImaginary: 0.025, phaseAtZero: -0.61 },
  ] as const;
  for (const speed of [0, 1e-10, 1e-4, 0.9, -2.1]) {
    const modes = coefficients.map((coefficient, index) => ({
      ...coefficient,
      angularSpeed: speed * (index + 1.25),
    }));
    const t0 = 0.13;
    const t1 = 1.67;
    const exact = integrateQuadraticWindowedRealFourierSeriesMoments(
      constant,
      modes,
      weight,
      t0,
    ...
    for (let index = 0; index < steps; index++) {
      const t = t0 + (index + 0.5) * dt;
      const window = weight[0] + weight[1] * t + weight[2] * t * t;
      let density = constant;
      for (const mode of modes) {
        const phase = mode.phaseAtZero + mode.angularSpeed * t;
        density += 2 * (
      ...
      const weightedDensity = window * density;
      opticalDepth += weightedDensity * dt;
      firstMoment += t * weightedDensity * dt;
    }
    close(exact.opticalDepth, opticalDepth, 3e-9);
    close(exact.firstMoment, firstMoment, 4e-9);
    close(exact.centroid!, firstMoment / opticalDepth, 4e-9);
  }
});
